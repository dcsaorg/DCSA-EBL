...
public class CargoItemController extends ExtendedBaseController<CargoItemService, CargoItem, UUID> {
    ...
    @GetMapping(path = "{id}")
    @Override
    public Mono<CargoItem> findById(@PathVariable  UUID id) {
    ...
    @Override
    public Mono<CargoItem> update(@PathVariable UUID id, @Valid @RequestBody CargoItem shipment) {
        return super.update(id, shipment);
    }

    @Override
    public Mono<CargoItem> create(@Valid @RequestBody CargoItem cargoItem) {
        return Mono.error(new CreateException("Not possible to create a Cargo Item"));
    }
}
