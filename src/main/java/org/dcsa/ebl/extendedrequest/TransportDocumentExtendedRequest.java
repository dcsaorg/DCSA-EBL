...
public class TransportDocumentExtendedRequest<T extends TransportDocument>
    extends ExtendedRequest<T> {

  private static final String CARRIER_BOOKING_REFERENCE_PARAMETER = "carrierBookingReference";

  public TransportDocumentExtendedRequest(
      ExtendedParameters extendedParameters, R2dbcDialect r2dbcDialect, Class<T> modelClass) {
  ...
  protected void markQueryFieldInUse(QueryField fieldInUse) {
    ...
    selectDistinct =
        selectDistinct || fieldInUse.getJsonName().equals(CARRIER_BOOKING_REFERENCE_PARAMETER);
  }
